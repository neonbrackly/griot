...
class ValidationEngine:
    ...
    def validate(
        ...
        *,
        ...
        if column_errors:
            ...
            if validation_mode == ValidationMode.EAGER:
                return self._build_result(
                    adapter, schema, errors, warnings, start_time
                )

        type_warnings = check_types(adapter, schema_fields)
        warnings.extend(type_warnings)

        # Phase 2: Quality rule evaluation
        # Field-level rules
        ...
        return self._build_result(adapter, schema, errors, warnings, start_time)
...
def validate_dataframe(
    ...
    *,
    ...
    return engine.validate(df, schema, fail_fast=fail_fast)
